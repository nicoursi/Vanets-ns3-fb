...
interactive_cleanup() {
  ...
  display_stats
  log "${YELLOW}Do you want to:${RESET}"
  log "  ${BOLD}1${RESET}) Kill everything, including Docker containers"
  log "  ${BOLD}2${RESET}) Kill parallel jobs only (leave Docker containers running)"
  log "  ${BOLD}anykey${RESET}) Defaults to choice 1"
  echo -ne "${BLUE}Your choice [1/2]: ${RESET}"

  local choice=""
  IFS= read -r choice < /dev/tty
  case "$choice" in
    2)
      log "${YELLOW}Leaving Docker containers running and killing parallel jobs...${RESET}"
      pkill -P $$
      ;;
    1|"")
      log "${YELLOW}Killing everything...${RESET}"
      killeverything
      exit 1
      ;;
    *)
      log "${RED}Invalid choice. Going for option 1.${RESET}"
      killeverything
      exit 1
      ;;
  esac
