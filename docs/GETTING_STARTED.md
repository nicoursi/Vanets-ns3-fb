...
- [3. Working on the Cluster](#3-working-on-the-cluster)
  - [3.1. Building the Project](#31-building-the-project)
  - [3.2. Generating and Submitting Jobs](#32-generating-and-submitting-jobs)
...
### 3.2. Generating and Submitting Jobs
...
```bash
./build_env/run_singularity_local.sh build
```

For subsequent code modifications it is enough to just run the simulation:

```bash
./build_env/run_singularity_local.sh run [simulation_command_file] [rng_run_value]
```

As the `run_singularity_local` script takes as input a `.job` file instead of a `slurm` one, you need to generate the job to run with the `--only-command` parameter passed to `generate_maps_and_jobs.py`.
...
# Run the simulation
../build_env/run_singularity_local.sh run urban-Grid-300-highBuildings0-drones0-d25-cw-32-1024-b0-e0-j0-STATIC-100-300-.job
```
