...
Examples:
  ...
  ./generateMapsAndJobsTemplate.py --jobArray "1-100" --printCoords
  
  # Custom contention windows (JSON format)
  ./generateMapsAndJobsTemplate.py --contentionWindows '[{{"cwMin": 16, "cwMax": 128}}, {{"cwMin": 32, "cwMax": 1024}}]'
  
  # Enable buildings and drones with custom transmission ranges
  ./generateMapsAndJobsTemplate.py --buildings "{','.join(default_buildings + ['0'])}" --drones "{','.join(default_drones + ['1'])}" --txRanges "{','.join(default_tx_ranges[:3])}"
  
  # Custom error rates and forged coordinate rates
  ./generateMapsAndJobsTemplate.py --errorRates "{','.join(default_error_rates + ['10', '20'])}" --forgedCoordRates "{','.join(default_forged_coord_rates + ['10', '20'])}"
  
  # Override resource requirements
  ./generateMapsAndJobsTemplate.py --ram "4G" --neededTime "02:30:00"
  ...
  ./generateMapsAndJobsTemplate.py --jobsPath "/path/to/custom/jobs/directory"
    ...
    # Optional parameters
    parser.add_argument(
        '--genLossFile', '-l', action='store_true',
    ...
    parser.add_argument(
        ...
        help='Path to the map file (optional, for manual mode)'
    )
    parser.add_argument(
        'vehicleDistance', nargs='?', default=None,
...
def find_num_nodes(mobility_file_path):
    ...
    with mobility_path.open('r') as file:
        ...
        for line in file:
            ...
            if len(split_line) == 4:
                node_id = int(
                    split_line[0].split("_")[1].replace("(", "").replace(")", "")
                )
                if node_id > max_id:
                    max_id = node_id
    return max_id + 1
        ...
        for cw in contention_windows:
            for scenario in scenarios:
                ...
                if "Grid" in scenario or "Platoon" in scenario:
                    run_scenario(
                        cw, scenario, "25", starting_node_map[scenario],
                        vehicles_number[scenario], job_array, print_coords, area, config
                    )
                else:
                    distance = scenario.split("-")[1]
                    run_scenario(
                        cw, scenario, distance, starting_node_map[scenario],
                        vehicles_number[scenario], job_array, print_coords, area, config
                    )
