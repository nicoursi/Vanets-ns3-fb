...
def printSingleGraphErrorRate(
    ...
    for prot in protocolsList:
        ...
        for errorRate in errorRates:
            ...
            metricConfIntList.append(
                compoundData[errorRate][junction][txRange][protocol][metricConfInt]
            )
        rects.append(
            (
                ax.bar(
                    ind + widthDistance[count] * barWidth,
                    metricMeanList,
                    barWidth,
                    color=colors[count],
                    label=prot,
                    yerr=metricConfIntList,
                    capsize=4,
                )
            )
        )
...
def appendCompoundData(
    ...
    for txRange in txRanges:
        for protocol in protocols:
            ...
            if protocol != "ROFF":
                path = os.path.join(
                    realBasePath, errorRate, "r" + txRange, "j" + junction, cw, protocolPath
                )
            else:
                roff = True
                path = os.path.join(
                    realBasePath, errorRate, "r" + txRange, "j" + junction, protocolPath
                )
...
def printGridComparison():
    ...
    basePathb0 = "../../simulations/Grid/b0"
    basePathb1 = "../../simulations/Grid/b1"
            ...
            for cw in cws:
                for junction in junctions:
                    ...
                    for metric in metrics:
                        ...
                        printSingleGraph(
                            graphOutFolder,
                            graphTitles[metric] + additionalTitleStr,
                            compoundData,
                            txRanges,
                            protocols,
                            cw,
...
def printDroneComparison():
    ...
    plt.rcParams["figure.figsize"] = [18, 14]
    initialBasePath = "../../simulations/scenario-droni"
    # scenarios = ["Grid-200", "Grid-300", "Grid-400", "LA-15", "LA-25", "LA-35", "LA-45", "Padova-15", "Padova-25", "Padova-35", "Padova-45"]
    scenarios = ["LA-25"]
    highBuildings = ["0", "1"]
    buildings = ["0", "1"]
            ...
            for building in myBuildings:
                for cw in cws:
                    for junction in junctions:
                        ...
                        graphOutFolder = os.path.join(
                            scenario, "drones", "b" + building + "-h" + highBuilding
                        )
                        for metric in metrics:
                            yLabel = metricYLabels[metric]
                            ...
                            printSingleGraph(
                                graphOutFolder,
                                graphTitles[metric] + additionalTitle[building][highBuilding],
                                compoundData,
                                txRanges,
                                protocols,
                                cw,
                        ...
                        for junction in junctions:
                            for errorRate in errorRates:
                                for txRange in txRanges:
                                    for protocol in protocols:
                                        metricMean = metric + "Mean"
                                        value = errorRateCompoundData[errorRate][junction][txRange][
                                            protocol
                                        ][metricMean]
                                        if value > maxMetricValues[metric]:
                                            maxMetricValues[metric] = value
                        ...
                        for junction in junctions:
                            for forgedRate in forgedRates:
                                for txRange in txRanges:
                                    for protocol in protocols:
                                        metricMean = metric + "Mean"
                                        value = forgedRateCompoundData[forgedRate][junction][
                                            txRange
                                        ][protocol][metricMean]
                                        if value > maxMetricValues[metric]:
                                            maxMetricValues[metric] = value
                ...
                for junction in junctions:
                    for distance in distances:
                        basePath = os.path.join(
                            initialBasePath, scenario + "-" + distance, "b" + building
                        )
                        compoundData = initCompoundData(txRanges, protocols, metrics)
                        appendCompoundData(
                            basePath,
                        ...
                        for junction in junctions:
                            for distance in distances:
                                for txRange in txRanges:
                                    for protocol in protocols:
                                        metricMean = metric + "Mean"
                                        value = distanceCompoundData[distance][junction][txRange][
                                            protocol
                                        ][metricMean]
                                        if value > maxMetricValues[metric]:
                                            maxMetricValues[metric] = value
...
def printOldFBComparison():
    ...
    initialBasePath = "../../simulations/scenario-urbano"
    alternativeInitialBasePath = "../../simulations/scenario-urbano-oldFB"
    # scenarios = ["Grid-200", "Grid-300", "Grid-400", "LA-15", "LA-25", "LA-35", "LA-45", "Padova-15", "Padova-25", "Padova-35", "Padova-45"]
    scenarios = ["LA-25"]
    buildings = ["1"]
    errorRate = "e0"
    ...
    for scenario in scenarios:
        ...
        for building in buildings:
            for cw in cws:
                for junction in junctions:
                    basePath = os.path.join(initialBasePath, scenario, "b" + building)
                    alternativeBasePath = os.path.join(
                        alternativeInitialBasePath, scenario, "b" + building
                    )
                    compoundData = initCompoundData(txRanges, protocols, metrics)
                    appendCompoundData(
                        basePath,
                ...
                for junction in junctions:
                    ...
                    for metric in metrics:
                        for td in tds:
                            ...
                            printSingleGraph(
                                graphOutFolder,
                                graphTitles[metric] + additionalTitle[building][junction],
                                compoundData,
                                txRanges,
                                myProtocols,
                                cw,
...
"""
def print_distance_comparison(cw, vehicleDistances, protocols, xList, xLabels, figurePrefix, graphTitleExtension, folder, decreaseConfInts=False):
	plt.rcParams["figure.figsize"] = [18, 10]
	basePath = os.path.join("../../simulations/scenario-urbano", cw, "Padova")
...
def printCwComparison(cws, vehicleDistance, protocols, xList, xLabels, figurePrefix, graphTitleExtension, folder, decreaseConfInts=False):
	plt.rcParams["figure.figsize"] = [18, 10]
	basePath = "../../simulations/scenario-urbano"
