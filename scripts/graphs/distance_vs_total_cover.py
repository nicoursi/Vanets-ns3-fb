        ...
        rects.append(
            (
                ax.bar(
                    ...
                    dataDictionary[protocol],
                    barWidth,
                    color=colors[count],
                    label=protocol,
                    yerr=confIntDictionary[protocol],
                    capsize=4,
                )
            )
        )
...
def printdistance_vs_tolal_cover(protocols, figurePrefix, graphTitleExtension):
    # Data to plot
    totCoverageMeans = {}
    totCoverageConfInts = {}
    covOnCircMeans = {}
    ...
    for distance in vehicleDistances:
        for protocol in protocols:
            ...
            data = graph_utils.read_csv_from_directory(path)
    ...
    print(hopsMeans)
    printSingleGraph(
        "Padua scenario with buildings, total coverage with varying vehicle distance ("
        + graphTitleExtension
        + ")",
        "Total coverage (%)",
    ...
    print("2")
    printSingleGraph(
        "Padua scenario with buildings, coverage on circumference with varying vehicle distance ("
        + graphTitleExtension
        + ")",
    ...
    printSingleGraph(
        "Padua scenario with buildings, number of hops with varying vehicle distance ("
        + graphTitleExtension
        + ")",
        "Number of hops",
    ...
    print("3")
    printSingleGraph(
        "Padua scenario with buildings, number of alert messages sent with varying vehicle distance ("
        + graphTitleExtension
        + ")",
