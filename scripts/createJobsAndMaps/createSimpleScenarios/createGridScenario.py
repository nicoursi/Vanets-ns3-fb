...
# Invocation:
#	./createGridScenario.py roadLength roadNumber roadDistance roadSize nodeDistance scenarioName [roadVariation] [nodeVariation]
# e.g.
...
#   ./createGridScenario.py 4800 17 300 10 25 Grid-300+-5-node+-5 5 5
...
def main():
	...
	with open("./grid/" + scenarioName + ".ns2mobility.xml", "w+") as f:
			...
			for x_idx in range(roadNumber):
				...
				while True:
					...
					if current_y <= initialY + roadLength:
						all_positions.append((road_x, current_y))
						print(f"Vertical road {x_idx}: Node at ({road_x}, {current_y}), distance: {varied_distance}")
					else:
						break
			...
			for y_idx in range(roadNumber):
				...
				while True:
					...
					if current_x <= initialX + roadLength:
						if (current_x, road_y) not in all_positions:
							all_positions.append((current_x, road_y))
							print(f"Horizontal road {y_idx}: Node at ({current_x}, {road_y}), distance: {varied_distance}")
					else:
						break
		else:
		...
		print(f"Grid bounds: X=[{initialX}, {initialX + roadLength}], Y=[{initialY}, {initialY + roadLength}]")
		print(f"Last road positions: X={verticalRoadDistances[-1]}, Y={horizontalRoadDistances[-1]}")
