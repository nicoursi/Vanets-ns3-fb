...
def get_options(args=None):
    ...
    optParser.add_option(
        ...
        type="float",
        default="50.0",
        help="define the distance between vehicles",
    )
...
def generate_one(idx, depart, departPos, laneFrom, laneTo):
    return '<trip id="%s" depart="%.2f" departPos="%d" from="%s" to="%s" type="car"/>' % (
        idx,
        depart,
        departPos,
        laneFrom,
...
def isFeasibleRoad(edge):
    ...
    return (
        edge.allows("custom1")
        or edge.allows("custom2")
        or edge.allows("passenger")
        or edge.allows("private")
    )
...
def main(options):
    ...
    with open(options.tripfile, "w") as fouttrips:
        ...
        fouttrips.write(
            '\t<vType id="car" accel="0.8" decel="4.5" sigma="0.5" length="4" maxSpeed="50"/>\n\n'
        )
