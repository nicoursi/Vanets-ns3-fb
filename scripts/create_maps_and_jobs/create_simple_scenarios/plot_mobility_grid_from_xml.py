...
def create_verification_plot(nodes, scenario_name="NS2 Mobility Verification", show_coords=False):
    ...
    print(
        f"Found {len(overlapping_nodes)} overlapping nodes at {len(set((pos['X'], pos['Y']) for _, pos in overlapping_nodes))} positions"
    )
    ...
    x_step, x_start, x_end = smart_tick_spacing(
        x_data_min, x_data_max, grid_spacing=x_grid_spacing if is_grid else None
    )
    y_step, y_start, y_end = smart_tick_spacing(
        y_data_min, y_data_max, grid_spacing=y_grid_spacing if is_grid else None
    )
    ...
    if is_grid:
        print(
            f"Grid mode - X-axis: range {x_data_min:.1f} to {x_data_max:.1f}, major ticks every {x_step:.1f}m"
        )
        print(
            f"Grid mode - Y-axis: range {y_data_min:.1f} to {y_data_max:.1f}, major ticks every {y_step:.1f}m"
        )
    else:
        print(
            f"Scatter mode - X-axis: range {x_data_min:.1f} to {x_data_max:.1f}, major ticks every {x_step:.1f}m"
        )
        print(
            f"Scatter mode - Y-axis: range {y_data_min:.1f} to {y_data_max:.1f}, major ticks every {y_step:.1f}m"
        )
    ...
    legend_elements = [
        plt.Line2D(
            ...
            markerfacecolor="red",
            markersize=4,
            label=f"Regular Nodes ({regular_nodes})",
        ),
    ]
    if overlapping_nodes:
        legend_elements.append(
            plt.Line2D(
                ...
                markersize=4,
                label=f"Overlapping Nodes ({len(overlapping_nodes)})",
            )
        )
...
def main():
    ...
    parser.add_argument(
        "-c", "--show-coords", action="store_true", help="Show coordinates above nodes"
    )
