...
def createJobFile(newJobName, command, jobsPath, jobTemplatePath, tempNewJobPath, ram, neededTime, jobArray):
...
def runScenario(cw, scenario, distance, startingNode, vehiclesNumber, jobArray, printCoords, area=1000):
	print(scenario)
							...
							for errorRate in errorRates:
								...
								if (protocol == "6"): #ROFF
									command = (
									    ...
									    f"\n  --buildings={b} \\"
									    f"\n  --actualRange={txRange} \\"
									    f"\n  --mapBasePath={mapPathWithoutExtension} \\"
									    f"\n  --vehicleDistance={distance} \\"
									    ...
									    f"\n  --errorRate={errorRate} \\"
									    f"\n  --nVehicles={vehiclesNumber} \\"
									    f"\n  --droneTest={drone} \\"
									    f"\n  --highBuildings={highBuilding} \\"
									    f"\n  --printToFile=1 \\"
									    f"\n  --printCoords={printCoords} \\"
									    f"\n  --createObstacleShadowingLossFile=0 \\"
									    f"\n  --useObstacleShadowingLossFile=1 \\"
									    f"\n  --beaconInterval=100 \\"
									    f"\n  --distanceRange=1 \\"
									    f"\n  --forgedCoordTest=0 \\"
									    f"\n  --forgedCoordRate=0 \\"
								...
								newJobName = (
								    ...
								    f"-e{errorRate}"
								    f"-j{junction}"
								    f"-{protocolsMap[protocol]}"
								    f"-{txRange}"
								    f"{'-with-coords' if printCoords == 1 else ''}"
								)
								createJobFile(newJobName, command, jobsPath, jobTemplatePath, tempNewJobPath, ram, neededTime, jobArray)
							'''
		...
		for cw in contentionWindows:
			for scenario in scenarios:
				...
				if ("Grid" in scenario or "Platoon" in scenario):
					runScenario(cw, scenario, "25", startingNodeMap[scenario], vehiclesNumber[scenario], jobArray, printCoords, area)
				else:
					distance = scenario.split("-")[1]
					runScenario(cw, scenario, distance, startingNodeMap[scenario], vehiclesNumber[scenario], jobArray, printCoords, area)
	else:
