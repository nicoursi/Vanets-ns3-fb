...
def plot_alert_paths(csv_file_path, output_file_path, config):
    ...
    node_bounds = coordUtils.calculateCoordBounds(
...
#        plt.plot(x_node_coords, y_node_coords, ".", markersize=5, color="red", alpha=0.3, label="All nodes")
        plt.plot(x_node_coords, y_node_coords, ".", color="red", alpha=0.6, label="Not receiving nodes")
        plt.plot(x_received_coords, y_received_coords, ".", color="green", alpha=0.5, label="Receiving nodes")

    # plot source
    plt.plot(starting_x, starting_y, "*", color="yellow", markersize=10, markeredgecolor='blue', markeredgewidth=1, label="Source node", zorder=10)

    colors = ['#FF0000', '#00FF00', '#0000FF', '#FF00FF', '#00FFFF', '#FF8000', '#8000FF', '#008000']
    sender_color_map = {}
    color_index = 0
    ...
    for sender, receivers in transmission_map.items():
        ...
        sender_coord = coordUtils.findCoordsFromFile(sender, config.mobility_file)
        if sender_coord is None:
            continue
        # mark sender as forwarder
        if sender not in plotted_forwarders:
            plt.plot(sender_coord.x, sender_coord.y, "o", color=path_color, markersize=6,
        ...
        for recv in receivers:
            recv_coord = coordUtils.findCoordsFromFile(recv, config.mobility_file)
            if recv_coord is None:
                continue
            if recv in transmission_map and transmission_map[recv]:
                plt.annotate('', xy=(recv_coord.x, recv_coord.y), xytext=(sender_coord.x, sender_coord.y),
                             arrowprops=dict(arrowstyle='->', color=path_color, lw=1.5, alpha=0.8,
                                             shrinkA=3, shrinkB=3, mutation_scale=15), zorder=5)
    ...
    if hasattr(config, 'debug') and config.debug:
        ...
        circumference_candidates = find_circumference_candidates(x_received_coords, y_received_coords, starting_x, starting_y, node_spacing, config)
        received_on_circ_ids_fallback = coordUtils.findNodeIdsFromCoords(circumference_candidates, config.mobility_file)
        if sorted(received_on_circ_ids_fallback)==sorted(received_on_circ_ids):
            print("✅ The receivers on circumference from the NS-3 simulation and the fallback correspond")
        else:
    ...
    if not received_on_circ_ids or received_on_circ_ids == ['0'] or received_on_circ_ids == [0]:
        ...
        print(f"Node spacing used to calculate the circumference: {node_spacing}. Change if different!\n")
        ...
        if config.debug:
            ...
            circumference_candidates = find_circumference_candidates(
                x_received_coords, y_received_coords, starting_x,
                starting_y, node_spacing, config)
            
            received_on_circ_ids = coordUtils.findNodeIdsFromCoords(
    ...
    for circ_id in received_on_circ_ids:
        for sender, receivers in transmission_map.items():
            if circ_id in receivers:
                last_forwarder_coord = coordUtils.findCoordsFromFile(sender, config.mobility_file)
                circ_coord = coordUtils.findCoordsFromFile(circ_id, config.mobility_file)
                if last_forwarder_coord is not None and circ_coord is not None:
                    plt.plot([last_forwarder_coord.x, circ_coord.x], [last_forwarder_coord.y, circ_coord.y],
                             'k-', linewidth=1.5, alpha=0.6, zorder=3)
                    plt.plot(circ_coord.x, circ_coord.y, '.', color="green", markersize=5, alpha=0.5, zorder=9)

    coordUtils.plotTxRange(config.circ_radius, starting_x, starting_y, vehicle_distance, color="#840000", plotInterval=True, coordBounds=coord_bounds)
    ...
    plt.plot([], [], "o", color="gray", markersize=6,
             markeredgecolor='black', markeredgewidth=1, label='Forwarding nodes')
    ...
    base_title = f'{config.scenario} (Transmission range: {tx_range}m) - Alert Message Propagation Paths'
    if simulation_bug_detected:
        title_color = 'red'
        plt.title(base_title, fontsize=14, color=title_color)
        # Add warning text
        plt.text(
            0.02, 0.02, bug_warning, transform=plt.gca().transAxes, 
    ...
    if not coordUtils.ensure_output_directory(output_file_path):
        return False
    try:
